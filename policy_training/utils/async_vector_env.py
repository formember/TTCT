...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_one_env(self, index,timeout=None):
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError('Calling `reset_async` while waiting '
                'for a pending call to `{0}` to complete'.format(
        ...
        if not successes:
            raise RuntimeError('Error in reset')
        self._state = AsyncState.DEFAULT
        observation,info=results
        
        return (deepcopy(observation) if self.copy else observation , info)
    ...
    def step_wait(self, timeout=None):
        ...
        return (deepcopy(self.observations) if self.copy else self.observations,
                np.array(rewards), np.array(costs), np.array(terminateds, dtype=np.bool_), np.array(truncateds, dtype=np.bool_), infos)
