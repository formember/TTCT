...
class Actor(nn.Module):
    ...
    def __init__(self, obs_dim: int, act_dim: int, hidden_sizes: list = [64, 64], is_discrete: bool = False):
        super().__init__()
    ...
    def forward(self, obs: torch.Tensor):
        ...
        if self.is_discrete:
            ...
            mean_norm = torch.tanh(mean)
            std = torch.exp(self.log_std)
            mean_norm_mul=mean_norm*torch.tensor([20,0.785],device='cuda')
            return Normal(mean_norm_mul, std)


class ActorTrajectory(nn.Module):
    """
    Actor network for policy-based reinforcement learning.
    ...
    def __init__(self, obs_dim: int, trajectory_dim, text_dim, act_dim: int, hidden_sizes: list = [64, 64], is_discrete: bool = False):
        super().__init__()
        self.obs_dim=obs_dim
        self.trajectory_dim=trajectory_dim
        self.text_dim=text_dim
        self.is_discrete = is_discrete
        self.text_encoder = build_mlp_network([text_dim]+hidden_sizes[0:2])
        self.trajectory_encoder = build_mlp_network([trajectory_dim]+hidden_sizes[0:2])
        self.obs_encoder = build_mlp_network([obs_dim]+hidden_sizes[0:2])
        self.mean = build_mlp_network([hidden_sizes[1]*3]+hidden_sizes[2:]+[act_dim])
        self.log_std = nn.Parameter(torch.zeros(act_dim), requires_grad=True)

    def forward(self, obs_tra_text):
        obs=obs_tra_text[:,:self.obs_dim]
        trajectory=obs_tra_text[:,self.obs_dim:self.obs_dim+self.trajectory_dim]
        text=obs_tra_text[:,self.obs_dim+self.trajectory_dim:]
        combined_feature=torch.cat([self.obs_encoder(obs),self.trajectory_encoder(trajectory),self.text_encoder(text)],dim=-1)
...
class VCriticTrajectory(nn.Module):
    """
    Critic network for value-based reinforcement learning.
    ...
    def __init__(self, obs_dim, trajectory_dim, text_dim, hidden_sizes: list = [64, 64]):
        super().__init__()
        self.obs_dim=obs_dim
        self.trajectory_dim=trajectory_dim
        self.text_dim=text_dim
        ...
        self.trajectory_encoder = build_mlp_network([trajectory_dim]+hidden_sizes[0:2])
        self.obs_encoder = build_mlp_network([obs_dim]+hidden_sizes[0:2])
        self.critic = build_mlp_network([hidden_sizes[1]*3]+hidden_sizes[2:]+[1])

    def forward(self,obs_tra_text):
        obs=obs_tra_text[:,0:self.obs_dim]
        trajectory=obs_tra_text[:,self.obs_dim:self.obs_dim+self.trajectory_dim]
        text=obs_tra_text[:,self.obs_dim+self.trajectory_dim:]
        combined_feature=torch.cat([self.obs_encoder(obs),self.trajectory_encoder(trajectory),self.text_encoder(text)],dim=-1)
        return torch.squeeze(self.critic(combined_feature), -1)
...
class ActorVCritic(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes: list = [256, 128, 64],is_discrete=False):
        super().__init__()
        # if isinstance(obs_dim, tuple):
        #     self.vision=nn.Sequential(
        #         nn.Conv2d(3, 128//4, kernel_size=3, stride=1, padding=1),
        #         nn.ReLU(),
        #         nn.Conv2d(128//4, 128//2, kernel_size=3, stride=1, padding=1),
        #         nn.ReLU(),
        ...
        self.actor = Actor(obs_dim, act_dim, hidden_sizes,is_discrete=is_discrete)
...
class ActorVCriticTrajectory(nn.Module):
    """
    Actor-critic policy for reinforcement learning.
    ...
    def __init__(self, obs_dim, trajectory_dim,text_dim,act_dim, hidden_sizes: list = [256, 128, 64],is_discrete=False):
        super().__init__()

        self.is_discrete = is_discrete
        self.reward_critic = VCriticTrajectory(obs_dim,trajectory_dim, text_dim,hidden_sizes)
        self.cost_critic = VCriticTrajectory(obs_dim,trajectory_dim,text_dim, hidden_sizes)
        self.actor = ActorTrajectory(obs_dim, trajectory_dim, text_dim, act_dim, hidden_sizes,is_discrete=is_discrete)
    ...
    def step(self, obs_tra_text,deterministic=False):
        """
        Take a policy step based on observations.
        ...
        if self.is_discrete:
            ...
            if deterministic:
                ...
                action = dist.rsample()
            log_prob = dist.log_prob(action).sum(axis=-1)
        value_r = self.reward_critic(obs_tra_text)
        value_c = self.cost_critic(obs_tra_text)
        return action, log_prob, value_r, value_c
