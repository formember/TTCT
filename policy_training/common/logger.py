...
class Logger:
    ...
    def setup_torch_saver1(self, what_to_save):
        """
        Set up easy model saving for a single PyTorch model.
        ...
        Args:
            what_to_save: Any PyTorch model or serializable object containing
                PyTorch models.
        """
        self.torch_saver_elements1= what_to_save
    ...
    def torch_save1(self, itr=None):
        """Saves the PyTorch model (or models)."""

        self.log("Save model to disk...")
        assert (
            self.torch_saver_elements1 is not None
        ), "First have to setup saving with self.setup_torch_saver"
        fpath = "torch_save"
        fpath = osp.join(self.log_dir, fpath)
        fname = "Encodemodel" + ("%d" % itr if itr is not None else "") + ".pt"
        fname = osp.join(fpath, fname)
        os.makedirs(fpath, exist_ok=True)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            torch.save(self.torch_saver_elements1, fname)
        torch.save(lora.lora_state_dict(self.torch_saver_elements),fname)
        self.log("Done.")
