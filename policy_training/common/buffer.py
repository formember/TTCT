...
class VectorizedOnPolicyBuffer:
        ...
        self.buffers: list[dict[str, torch.tensor]] = [
            {
                ...
                "obslist": [[] for i in range(size)],
                "actlist": [[] for i in range(size)],
                "mission": ["" for i in range(size)],
                "lengths": [0 for i in range(size)],
    ...
    def finish_path(
        ...
        if TL_condition:
            path_len=self.ptr_list[idx]-self.path_start_idx_list[idx]
            with torch.no_grad():
                self.buffers[idx]["cost"][path_slice] = TLmodel.get_cost([obslist],[actlist],emb_mission,use_cost_prediction)[-path_len:]
        ...
        return self.buffers[idx]["cost"][path_slice] if TL_condition else None
    ...
    def get(self) -> dict[str, torch.Tensor]:
        ...
        """
        avoid_key=["obslist","actlist","mission","lengths"]
        ...
        for k, v in data_pre.items():
            if k not in avoid_key:
                data[k]=torch.cat(v, dim=0)
            else:
                data[k]=[ele for item in v for ele in item]
